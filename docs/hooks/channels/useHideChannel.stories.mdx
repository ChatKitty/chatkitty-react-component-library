...
<Meta title="Hooks/Channels/useHideChannel" />
...
```typescript
const { resource, isLoading, error, makeRequest } = useHideChannel();
```
...
| :------------ | :------------------------------------------ | :--------------------------------------------- |
| `resource`    | Channel                                     | Channel that was hidden                        |
| `isLoading `  | boolean                                     | Request loading state                          |
| `error`       | ChatKittyError                              | If there's an error, it will be available here |
| `makeRequest` | `(channel: DirectChannel) => Promise<void>` | Callback to request or re-request              |
